/**
 * Country calling codes for the RSVP form's phone field.
 *
 * The whole world rather than a shortlist, and that is a deliberate trade. The
 * list costs roughly 9 kB of static HTML — no JavaScript, so it is outside the
 * form's script budget entirely — and it buys the guarantee that nobody opens
 * this form and finds their own country missing. A guest who cannot pick their
 * code cannot reply at all, and there is no second chance to notice.
 *
 * `iso` is the option's value rather than the dial code, because dial codes are
 * not unique: +1 is Canada and the United States, +7 is Kazakhstan and Russia,
 * +599 is Curaçao and more besides. Two <option>s sharing a value make
 * `select.value` ambiguous to read and impossible to set. The ISO 3166-1 alpha-2
 * code is unique by construction, and the dial code rides along in `dial`.
 *
 * `dial` is digits only — the leading `+` is added when the number is assembled
 * into E.164, so the string here is never half a phone number.
 *
 * Order in this file is not the order on screen. The component pins India and
 * Switzerland to the top and sorts the remainder with `localeCompare`, which is
 * the only thing that gets the accented names (Côte d'Ivoire, Curaçao, Réunion,
 * São Tomé and Príncipe, Türkiye) into the places an English reader expects.
 * Sorting in the frontmatter means it happens once at build, not per visitor.
 *
 * Crown dependencies (Guernsey, Jersey, Isle of Man) are deliberately absent:
 * all three dial as +44 and listing them only adds three ways to pick the same
 * answer as United Kingdom.
 */
